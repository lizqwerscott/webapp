...
(defun check-all-table ()
  (maphash #'(lambda (k v)
               (map 'vector #'(lambda (table)
                                (check-table table)) v)) *table-manager-hash*))
